...
export default function ShareButtons({ url, title, price, discount, productId }: ShareButtonsProps) {
  ...
  const shareText = `🔥 ${title} por ${price}${discount ? ` (-${discount}%)` : ""}\n\n👉 Veja no PromoSnap: ${url}`;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      {/* WhatsApp — principal e destaque */}
      <a
        href={whatsappUrl}
        target="_blank"
        rel="noopener noreferrer"
        onClick={() => trackShare("whatsapp")}
        className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#25D366] hover:bg-[#20BD5A] text-white text-sm font-semibold transition-colors shadow-sm"
      >
