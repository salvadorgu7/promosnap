      ...
      <form onSubmit={handleSubmit}>
        <div className={`relative flex items-center rounded-xl border transition-all duration-200 ${
          ...
          <input
            ...
            className={`flex-1 bg-transparent outline-none px-3 text-surface-800 placeholder:text-surface-400 ${large ? "text-lg" : "text-sm"}`}
          />
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTIONS.map((s) => (
              <button key={s} type="button"
                ...
                className="px-3 py-1.5 rounded-lg bg-surface-100 text-sm text-surface-600 hover:text-accent-blue hover:border-accent-blue/30 border border-transparent transition-colors">
                {s}
              </button>
            ))}
          </div>
