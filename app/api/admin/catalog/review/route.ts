...
// GET /api/admin/catalog/review — Unified review queue across all sources
export async function GET(req: NextRequest) {
  ...
  const source = req.nextUrl.searchParams.get('source') || null // null = all sources
  const scoreMin = parseInt(req.nextUrl.searchParams.get('scoreMin') || '0', 10)
  const scoreMax = parseInt(req.nextUrl.searchParams.get('scoreMax') || '100', 10)
  ...
  try {
    ...
    const [candidates, total, sourceCounts] = await Promise.all([
      ...
      prisma.catalogCandidate.count({ where }),
      // Aggregate counts per source for the filter UI
    ...
    const items = candidates
      .map(c => {
        ...
        const score = enriched?.score ?? enriched?.total ?? null
        return {
          id: c.id,
          title: c.title,
          price: c.price,
          ...
          marketplace: enriched?.marketplace ?? c.sourceSlug ?? null,
      ...
      .filter(item => {
        if (item.score == null) return scoreMin === 0 // Include items without score if min is 0
        return item.score >= scoreMin && item.score <= scoreMax
      })

    const sources = sourceCounts.map(s => ({
      slug: s.sourceSlug || 'unknown',
      count: s._count.id,
    }))
    ...
    logger.error('catalog-review.get-error', { error: String(err) })
    return NextResponse.json(
...
// PATCH /api/admin/catalog/review — Approve or reject candidates (any source)
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    // Fetch candidates — no source filter, works across all sources
    ...
    if (candidates.length === 0) {
      return NextResponse.json(
        { error: 'No pending candidates found with given IDs' },
        { status: 404 }
      )
    }
    ...
    if (action === 'approve') {
      const importItems: ImportItem[] = candidates.map(c => {
        ...
        return {
          ...
          discoverySource: c.sourceSlug || 'manual',
        }
      })
      ...
      if (validItems.length > 0) {
        try {
          ...
          await prisma.catalogCandidate.updateMany({
            where: { id: { in: candidates.filter(c => {
              const enriched = c.enrichedData as Record<string, any> | null
      ...
      // Mark items that couldn't be imported as APPROVED
    ...
    logger.info('catalog-review.action', {
      action,
    ...
    logger.error('catalog-review.patch-error', { error: String(err) })
    return NextResponse.json(
